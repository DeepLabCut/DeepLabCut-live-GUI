...
class CameraProcess(object):
    ...
    def __init__(self,
        ...
        self.frame = np.frombuffer(self.frame_shared.get_obj(), dtype='uint8').reshape(res[1], res[0], 3)
    ...
    def start_capture_process(self, timeout=60):
        ...
        self.capture_process = self.ctx.Process(target=self._run_capture,
    ...
    def start_writer_process(self, filename, timeout=60):
        ...
        self.writer_process = self.ctx.Process(target=self._run_writer,
                                               args=(filename,),
                                               daemon=True)
    ...
    def get_display_frame(self):
        ...
        if frame is not None:
            if self.device.display_resize != 1:
                frame = cv2.resize(frame, (int(frame.shape[1]*self.device.display_resize), int(frame.shape[0]*self.device.display_resize)))
