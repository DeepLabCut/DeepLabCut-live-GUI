...
"""OpenCV based camera backend with MJPG enforcement for WSL2."""
...
class OpenCVCameraBackend(CameraBackend):
    """Fallback backend using :mod:`cv2.VideoCapture` with MJPG optimization."""
    ...
    def _configure_capture(self) -> None:
        ...
        # Use MJPG if available
        if not self._capture.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*'MJPG')):
            LOG.warning("Failed to set MJPG format, falling back to default")

        # Log actual codec
        fourcc = int(self._capture.get(cv2.CAP_PROP_FOURCC))
        codec = "".join([chr((fourcc >> 8 * i) & 0xFF) for i in range(4)])
        LOG.info(f"Camera using codec: {codec}")
        ...
        if actual_width != width or actual_height != height:
            LOG.warning(f"Resolution mismatch: requested {width}x{height}, got {actual_width}x{actual_height}")
            try:
                self._resolution = (actual_width, actual_height)
            except Exception:
                LOG.warning("Failed to update internal resolution state")
