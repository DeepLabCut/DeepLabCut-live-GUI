...
class DLCLiveGUI(object):
    ...
    def init_cam(self):
        ...
        if not this_cam:
            ...
            messagebox.showerror("No Camera", "No camera selected. Please select a camera before initializing.", parent=self.window)
    ...
    def monitor_cam_processes(self):
        ...
        if self.cam:
            msg = self.cam.monitor_processes()
            if msg is not None:
                if msg[0] == "Capture":
                    self.cam_is_open = msg[1]
                elif msg[0] == "Pose":
                    self.pose_is_open = msg[1]
                elif msg[0] == "Writer":
                    self.writer_is_open = msg[1]
            self.window.after(10, self.monitor_cam_processes)
    ...
    def edit_dlc_settings(self, new=False):
        ...
        Label(self.dlc_settings_window, text="Name: ").grid(sticky='w', row=cur_row, column=0)
        self.dlc_settings_name = StringVar(self.dlc_settings_window, value=cur_set['name'])
        Entry(self.dlc_settings_window, textvariable=self.dlc_settings_name).grid(sticky='nsew', row=cur_row, column=1)
        cur_row += 1

        Label(self.dlc_settings_window, text="Model Path: ").grid(sticky='w', row=cur_row, column=0)
        self.dlc_settings_model_path = StringVar(self.dlc_settings_window, value=cur_set['model_path'])
        Entry(self.dlc_settings_window, textvariable=self.dlc_settings_model_path).grid(sticky='nsew', row=cur_row, column=1)
        Button(self.dlc_settings_window, text="Browse", command=self.browse_dlc_path).grid(sticky='nsew', row=cur_row, column=2)
        cur_row += 1

        Label(self.dlc_settings_window, text="Model Type: ").grid(sticky='w', row=cur_row, column=0)
        self.dlc_settings_model_type = StringVar(self.dlc_settings_window, value=cur_set['model_type'])
        Combobox(self.dlc_settings_window, textvariable=self.dlc_settings_model_type, value=['base', 'tensorrt', 'tflite'], state='readonly').grid(sticky='nsew', row=cur_row, column=1)
        cur_row += 1

        Label(self.dlc_settings_window, text="Precision: ").grid(sticky='w', row=cur_row, column=0)
        self.dlc_settings_precision = StringVar(self.dlc_settings_window, value=cur_set['precision'])
        Combobox(self.dlc_settings_window, textvariable=self.dlc_settings_precision, value=['FP32', 'FP16', 'INT8'], state='readonly').grid(sticky='nsew', row=cur_row, column=1)
        cur_row += 1

        Label(self.dlc_settings_window, text="Cropping: ").grid(sticky='w', row=cur_row, column=0)
        self.dlc_settings_cropping = StringVar(self.dlc_settings_window, value=cur_set['cropping'])
        Entry(self.dlc_settings_window, textvariable=self.dlc_settings_cropping).grid(sticky='nsew', row=cur_row, column=1)
        cur_row += 1

        Label(self.dlc_settings_window, text="Dynamic: ").grid(sticky='w', row=cur_row, column=0)
        self.dlc_settings_dynamic = StringVar(self.dlc_settings_window, value=cur_set['dynamic'])
        Entry(self.dlc_settings_window, textvariable=self.dlc_settings_dynamic).grid(sticky='nsew', row=cur_row, column=1)
        cur_row += 1

        Label(self.dlc_settings_window, text="Resize: ").grid(sticky='w', row=cur_row, column=0)
        self.dlc_settings_resize = StringVar(self.dlc_settings_window, value=cur_set['resize'])
    ...
    def update_dlc_settings(self):
        ...
        precision = self.dlc_settings_precision.get() if self.dlc_settings_precision.get() else 'FP32'
    ...
    def init_dlc(self):
        ...
        self.cam.set_dlc_params(dlc_params)
    ...
    def save_config(self, notify=False):
        ...
        if notify:
            messagebox.showinfo(title="Config file saved", message="Configuration file has been saved...", parent=self.window)


    def remove_cam_cfg(self):

        if self.camera_name.get() != "Add Camera":
            delete = messagebox.askyesno(title="Delete Camera?", message="Are you sure you want to delete '%s'?" % self.camera_name.get(), parent=self.window)
            if delete:
    ...
    def init_session(self):
        ...
        if self.record_on.get() > -1:
            messagebox.showerror("Video Open", "Video is currently open! Please release the current video (click 'Save Video' of 'Delete Video', even if no frames have been recorded) before setting up a new one.", parent=self.window)
            return 
        ...
        self.base_name = self.camera_name.get().replace(" ", "") + '_' + self.subject.get() + '_' + date + '_' + self.attempt.get()
        ...
        if os.path.isfile(self.vid_file) or os.path.isfile(self.ts_file) or os.path.isfile(self.dlc_file) or os.path.isfile(self.proc_file):
            overwrite = messagebox.askyesno("Files Exist", "Files already exist with attempt number = {}. Would you like to overwrite the file?".format(self.attempt.get()), parent=self.session_setup_window)
            if not overwrite:
                return

        ### open writer process

        self.cam.set_video_file(self.vid_file)
        self.cam.open_writer()
    ...
    def start_record(self):
        ...
        if self.writer_is_open:
            ret = self.cam.start_write()
        else:
            ret = False
    ...
    def save_vid(self, delete=False):
        ...
        self.cam.close_writer()

        ### perform checks ###

        if not self.cam:
            messagebox.showwarning("No Camera", "Camera has not yet been initialized, no video recorded.", parent=self.window)
            return

        elif not self.vid_file:
            messagebox.showwarning("No Video File", "Video was not set up, no video recorded.", parent=self.window)
            return
            ...
            ### get frame times. if no frames recorded, delete files

            frame_times = self.cam.get_frame_times()
            if not frame_times:
                messagebox.showwarning("No Frames Recorded", "No frames were recorded, video will be deleted", parent=self.window)
                delete = True
        ...
        if delete:
            ...
            messagebox.showinfo("Video Deleted", "Video, timestamp, DLC, and processor files have been deleted.", parent=self.window)

        else:

            # save timestamps as pickle file. Includes frame_times, ttl_in_times, and ttl_out_times
            pickle.dump({'frame' : frame_times}, open(self.ts_file, 'wb'))

            # save deeplabcut data as pandas data frame (same format as deeplabcut.analyze_videos)
            pose_df = self.cam.get_pose_df()
            if pose_df is not None:
                pose_df.to_hdf(self.dlc_file, key='df_with_missing', mode='w')

            # save pickle file with data from processor
            if self.proc:
    ...
    def createGUI(self):
        ...
        Button(self.window, text="Close Camera", command=self.close_camera).grid(sticky="nsew", row=cur_row, column=2)
        cur_row += 1
        ...
        Button(self.window, text='Edit DLC Settings', command=self.edit_dlc_settings).grid(sticky='nsew', row=cur_row, column=1)
        Button(self.window, text="Stop DLC", command=self.stop_pose).grid(sticky='nsew', row=cur_row, column=2)
        cur_row += 1
        ...
        # set up session
        Button(self.window, text="Set Up Session", command=self.init_session).grid(sticky="nsew", row=cur_row, column=1)
        cur_row += 2
        ...
        Radiobutton(self.window, text="Ready", selectcolor="blue", indicatoron=0, variable=self.record_on, value=0).grid(stick="nsew", row=cur_row, column=1)
        Radiobutton(self.window, text="On", selectcolor='green', indicatoron=0, variable=self.record_on, value=1, command=self.start_record).grid(sticky="nsew", row=cur_row+1, column=1)
